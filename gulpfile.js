...
const copy = async () => {
  ...
  else await execAsync("mv build dist/app");
};
...
const compileReact = async () => {
  return execAsync(
    `npx cross-env NODE_ENV=${process.env.BUILD_ENV} CI=true BUILD_PATH=./build react-scripts build`
  );
};
...
const devReact = async () => {
  return execAsync(
    `npx cross-env NODE_ENV=${process.env.BUILD_ENV} BROWSER=none PORT=3000 react-scripts start`
  );
};
...
const devElectron = async () => {
  if (process.env.BUILD_ENV === "production")
    return execAsync(
      `npx cross-env NODE_ENV=${process.env.BUILD_ENV} electron-forge start`
    );
...
const startElectron = async () => {
  return execAsync(
    `npx cross-env NODE_ENV=${process.env.BUILD_ENV} electron-forge start --app-path ./dist/main.js`
  );
};
...
const cleanup = async () => {
  ...
  } else await execAsync("rm -rf build dist out");
};

module.exports = {
  "start-dev": gulp.series(
    setEnv("development"),
    compileElectron,
    gulp.parallel(devReact, devElectron)
  ),
  "start-prod": gulp.series(
    setEnv("production"),
    compileReact,
    compileElectron,
    copy,
